 ...
 */
bool BLEClient::open(BLEAddress address, esp_ble_addr_type_t type)
{
	ESP_LOGD(LOG_TAG, ">> open(%s)", address.toString().c_str());
	...
	uint32_t rc = m_semaphoreOpenEvt.wait("open"); // Wait for the connection to complete.
	ESP_LOGD(LOG_TAG, "<< open(), rc=%d", rc == ESP_GATT_OK);

	return rc == ESP_GATT_OK;
}

/**
 * @brief Close from the peer.
 * @return N/A.
 */
void BLEClient::close()
{
	ESP_LOGD(LOG_TAG, ">> close()");
...
void BLEClient::gattClientEventHandler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *evtParam)
{
	...
	ESP_LOGD(LOG_TAG, "%s [esp_gatt_if: %d]", BLEUtils::gattClientEventTypeToString(event).c_str(), gattc_if);
	...
	case ESP_GATTC_OPEN_EVT:
	{
		...
		{
			m_isOpened = true; // Flag us as connected.
		}
		if (m_pClientCallbacks != nullptr)
		{
			m_pClientCallbacks->onOpen(this);
		}
	...
	case ESP_GATTC_CLOSE_EVT:
	{
		...
		if (m_pClientCallbacks != nullptr)
		{
			m_pClientCallbacks->onClose(this);
		}
	}
	break;
	...
	{
		// BLE physical connection connected
		m_isConnected = true;
	...
	{
		// BLE physical connection disconnected
		ESP_LOGE(__func__, "disconnect event, getConnId(): %d, conn_id: %d", getConnId(), evtParam->disconnect.conn_id);
		if (getConnId() != evtParam->disconnect.conn_id)
			break;

		m_semaphoreOpenEvt.give(evtParam->disconnect.reason);
		ESP_LOGD(__func__, "Currently, before change, m_isConnected is: %d", m_isConnected);

		// If we receive a disconnect event, set the class flag that indicates that we are no longer connected.
		if (!m_isConnected)
			break;
		m_isConnected = false;
		...
		// if (0)
		{
			...
			// m_semaphoreOpenEvt.take("273");
			esp_ble_gattc_close(m_gattc_if, m_conn_id);
			// m_semaphoreOpenEvt.wait("275");
		}

		if (0)
		{
			BLEDevice::removePeerDevice(m_appId, true);
			// m_semaphoreRegEvt.take("disconnect_evt");
			esp_ble_gattc_app_unregister(m_appId);
			// m_semaphoreRegEvt.wait("disconnect_evt");
			ESP_LOGD(__func__, "unregistered m_appId: %d", m_appId);
		}
...
{
	ESP_LOGD(LOG_TAG, ">> uuid: %s", uuid.toString().c_str());
	if (!isOpened())
	{
		if (!isConnected())
			ESP_LOGE(LOG_TAG, "// not connected");

		ESP_LOGE(LOG_TAG, "<< not opened");
		return nullptr;
	}
	// Design
	// ------
	...
	for (auto &myPair : m_servicesMap)
	{
		...
		{
			ESP_LOGD(LOG_TAG, "<< found the service with uuid: %s", uuid.toString().c_str());
			return myPair.second;
		}
	} // End of each of the services.
	ESP_LOGD(LOG_TAG, "<< not found");
	return nullptr;
}
...
{
	ESP_LOGD(LOG_TAG, ">> serviceUUID: %s, characteristicUUID: %s", serviceUUID.toString().c_str(), characteristicUUID.toString().c_str());
