...
void BLERemoteDescriptor::writeValue(uint8_t *data, size_t length, bool response)
{
	...
	ESP_LOGE(LOG_TAG, "esp_ble_gattc_write_char_descr.getGattcIf: 0x%x", m_pRemoteCharacteristic->getRemoteService()->getClient()->getGattcIf());
	ESP_LOGE(LOG_TAG, "esp_ble_gattc_write_char_descr.getConnId: 0x%x", m_pRemoteCharacteristic->getRemoteService()->getClient()->getConnId());
	ESP_LOGE(LOG_TAG, "esp_ble_gattc_write_char_descr.handle: 0x%x", getHandle());
	// ESP_LOG_BUFFER_HEX(LOG_TAG, data, length, ESP_LOG_ERROR);
	ESP_LOG_BUFFER_HEXDUMP(LOG_TAG, data, length, ESP_LOG_ERROR);
	ESP_LOGE(LOG_TAG, "esp_ble_gattc_write_char_descr.length: 0x%x", length);
	ESP_LOGE(LOG_TAG, "esp_ble_gattc_write_char_descr.data: 0x%x", data);
