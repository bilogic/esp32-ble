...
void BLERemoteCharacteristic::gattClientEventHandler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *evtParam)
{
	...
	ESP_LOGD(LOG_TAG, "%s [esp_gatt_if: %d]",
			 BLEUtils::gattClientEventTypeToString(event).c_str(), gattc_if);
...
void BLERemoteCharacteristic::registerForNotify(notify_callback notifyCallback, bool notifications)
{
	...
	ESP_LOGD(LOG_TAG, ">> %s", toString().c_str());
	...
	{
		// If we have a callback function, then this is a registration.
	...
	{
		// If we weren't passed a callback function, then this is an unregistration.
...
std::string BLERemoteCharacteristic::toString()
{
	...
	ss << "Characteristic: uuid: " << m_uuid.toString() << ", handle: " << getHandle() << " 0x" << std::hex << getHandle() << ", props: " << BLEUtils::characteristicPropertiesToString(m_charProp);
	return ss.str();
} // toString
