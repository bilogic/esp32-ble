...
class BLEClient
{
  public:
	...
	~BLEClient();
	bool app_register();
	bool open(BLEAdvertisedDevice *device);
	bool open(BLEAddress address, esp_ble_addr_type_t type = BLE_ADDR_TYPE_PUBLIC); // Connect to the remote BLE Server
	void close();																	// Disconnect from the remote BLE Server
	...
	bool isOpened();	// Return true if we are connected.
...
class BLEClientCallbacks
{
  public:
	...
	virtual void onOpen(BLEClient *pClient) = 0;
	virtual void onClose(BLEClient *pClient) = 0;
};
